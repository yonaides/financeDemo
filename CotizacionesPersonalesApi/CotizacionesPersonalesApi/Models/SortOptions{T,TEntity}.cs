...
namespace CotizacionesPersonalesApi.Models
{
    public class SortOptions<T, TEntity> : IValidatableObject
    {
        ...
        // ASP .NET CORE call this validate incoming parameters
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            var processor = new SortOptionsProcessor<T, TEntity>(OrderBy);

            var validTerms = processor.GetAllTerms().Select(x => x.Name);
        ...
        // The service  code will this to apply these sort options to a database query
